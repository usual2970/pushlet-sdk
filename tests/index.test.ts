...
describe("PushletClient", () => {
  let client: PushletClient;

  beforeEach(() => {
    client = new PushletClient("https://api.example.com/events");
    jest.clearAllMocks();
  });
  ...
  test("constructor should remove trailing slash from baseUrl", () => {
    const clientWithSlash = new PushletClient(
      "https://api.example.com/events/"
    );
    const clientWithoutSlash = new PushletClient(
      "https://api.example.com/events"
    );
    ...
    expect(sourceWithSlash.url).toBe(
      "https://api.example.com/events?topic=test"
    );
    expect(sourceWithoutSlash.url).toBe(
      "https://api.example.com/events?topic=test"
    );
  });
  ...
  test("subscribe should handle URL encoding for topic names", () => {
    ...
    expect(source.url).toBe(
      "https://api.example.com/events?topic=test%2Ftopic%20with%20spaces"
    );
  });
  ...
  test("subscribe should warn if already subscribed to topic", () => {
    ...
    expect(console.warn).toHaveBeenCalledWith(
      'Already subscribed to topic "test-topic"'
    );
  });
